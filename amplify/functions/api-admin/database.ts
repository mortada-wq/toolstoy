...
export async function query<T extends QueryResultRow = any>(text: string, values?: unknown[]): Promise<T[]> {
  const pool = getPool()
  const result = await pool.query<T>(text, values)
  return result.rows
}

export async function queryOne<T extends QueryResultRow = any>(text: string, values?: unknown[]): Promise<T | null> {
